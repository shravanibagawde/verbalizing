...
function splitLetters(paragraph, containerId) {
      const container = document.getElementById('text-container');
      const letters = Array.from(paragraph);

      const spanElements = letters.map((letter, index) => {
        const spanId = `span-${index + 1}`;
        return `<span id="${spanId}">${letter}</span>`;
      });
...
splitLetters(paragraph, 'container');
